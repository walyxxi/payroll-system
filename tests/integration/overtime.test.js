...
describe("/api/overtime", () => {
  ...
  beforeAll(async () => {
    ...
    if (userIds.length > 0) {
      ...
      await prisma.overtime.deleteMany({ where: { user_id: { in: userIds } } });
    ...
    // Create a payroll period for overtime
  ...
  it("should submit overtime with valid token and body", async () => {
    ...
    expect(res.body.hours).toBe("3");
    expect(res.body.description).toBe("Worked late");
  });
  ...
  it("should fail for wrong role", async () => {
    ...
    const adminUsername = "overtimeadmin";
